...
#if ext_only then will only extract files that have end in '*.*' => useful for IMGT zipped files that may have an individual files folder 
#move_all_files_to_folder_path => if True, then any files created in any subdirectories will be moved to the 'folder_path', all subdirectories will be subsequently deleted
def unzip_files_in_folder(folder_path,ext_only=True,move_all_files_to_folder_path=True):	
	...
	def use_untar(parent_folder,tar_file,decompress='',keep_zip_filename=False):
		...
		for x in os.listdir(tempfolder):
			if x.endswith('.txt'):
				p = tempfolder+'/'+x[:-4]
				new_name = p+'_'+file_substring+'.txt'				
				os.rename(p+'.txt',new_name)
				
			
		if move_all_files_to_folder_path:
			#recursively find all files and move them 
			os.system("find '{0}' -type f -exec mv {{}} '{1}' \;".format(tempfolder,parent_folder))
		else:
			#just move the results from the tar
			os.system("mv '{0}'/* '{1}' ".format(tempfolder,parent_folder))			
		#delete temp folder 
	...
	if tar_xz:		
		for each_tarxz in tar_xz:
			...
			use_untar(folder_path,each_tarxz,'J',keep_zip_filename=True)	
	...
	#GUNZIP
	if gz_files:		
		os.system('gunzip {0}'.format(' '.join(["'"+folder_path+f+"'" for f in gz_files])))
	
	#BUZNIP
	if bz_files:		
		os.system('bzip2 -d {0}'.format(' '.join(["'"+folder_path+f+"'" for f in bz_files])))
