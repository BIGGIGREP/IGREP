...
ms_db_filesets = os.path.join(os.path.dirname(os.path.relpath(__file__)), "msdb")
clonotype_location = os.path.join(program_folder, 'CDR3_Clonotyping')
...
def generate_msdb_file(input_files, filetype=None, output_folder_path=None, dbidentifier=None, dataset_tags=None, translate_fields={}, cluster_id=0.9, must_be_present=['CDR1', 'CDR2', 'CDR3'], use_vl_sequences=False, pc_file_location=None, vl_file_location=None, low_read_count=1):
	...
	if vdj_df is None:
		...
		clonotype_seqs(vdj_df, cluster_id, prefix + 'msDB.fasta', prefix + 'malGucken.txt', append=False, low_read_count=low_read_count)
		if low_read_count > 1:		
			print('Clonotyping heavy chains. Again.')
			clonotype_seqs(vdj_df, cluster_id, prefix + 'msDB.noFilter.fasta', prefix + 'malGucken.noFilter.txt', append=False)		
			# used below when adding extra sequences
			vdj_set_nf = open(prefix + 'msDB.nofilter.fasta', 'a')
		else:
			vdj_set_nf = None
	if not(vj_df is None):
		...
		# CONCATENATE  VL DATASET FROM OUR COMMON TOOL FOLDER
		if vl_file_location is None:
			vl_file_location = os.path.join(ms_db_filesets, "VL_set.fna")
		elif not(os.path.isfile(vl_file_location)):
			print('WARNING: THE PROVIDED VL FILE ({0}) DOES NOT EXIST. USING THE DEFAULT LOCATION'.format(vl_file_location))
			vl_file_location = os.path.join(ms_db_filesets, "VL_set.fna")
		with open(prefix + 'msDB.fasta', 'a') as vdj_set, open(vl_file_location) as vl_backup:
			for l in vl_backup:
				vdj_set.write(l)
				if not(vdj_set_nf is None):
					# also print seqs to no filter file
					vdj_set_nf.write(l)

	# CONCATENATE PROTEIN DATASET FROM COMMON TOOLS FOLDER
	print('Concatenating reference protein sequences to file')	
	if pc_file_location is None:
		pc_file_location = os.path.join(ms_db_filesets, "random_proteins.fna")
	elif not(os.path.isfile(pc_file_location)):
		print('WARNING: THE PROVIDED PROTEIN FILE ({0}) DOES NOT EXIST. USING THE DEFAULT LOCATION'.format(pc_file_location))
		pc_file_location = os.path.join(ms_db_filesets, "random_proteins.fna")
	with open(prefix + 'msDB.fasta', 'a') as vdj_set, open(pc_file_location) as proteins:
		for l in proteins:
			vdj_set.write(l)	
			if not(vdj_set_nf is None):
				# also print seqs to no filter file
...
def clean(df):
	...
	df['ABSEQ.AA'] = df['ABSEQ.AA'].str.replace('_', 'X')
	df['CDR3.AA'] = df['CDR3.AA'].str.replace('_', 'X')
...
def parse_annotation_files(input_files, filetype, fields, must_be_present, dataset_tags, dbidentifier, use_vl_sequences):
	...
	print('Sortingdata')
	df_alldata = df_alldata.sort_values(['READS'], ascending=False)
