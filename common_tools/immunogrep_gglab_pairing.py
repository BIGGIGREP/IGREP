...
import immunogrep_filesystem_tools as filesystem
#for reading files
import immunogrep_immunogrepfile as readfile
#import this function. However function is now present in immunogrepfile module, so this import can be deprecated in future
from cchrysostomou_imgt_reader_temp import GroupIMGTFiles
...
def gene_hist(d,filename,geneCall):
	...
	total=0
	for key in d:
		gene=d[key][1].split(":")[genePos].split("*")[0]
		if gene in hist:
			hist[gene]+=1
		else:
			hist[gene]=1
...
def ProcessGene(gene):
	...
	for gene_num,each_gene in enumerate(gene_array):
		...
		subv  = each_gene.split(' ')
		if len(subv)==1: #there is only one word 
			gene_array[gene_num] = subv.split('*')[0]
		else:
			g = ''
			for subv in each_gene.split(' '):
...
def parse_sorted_paired_file(pairing_temp_file):
	...
	my_folder = '/'.join(pairing_temp_file.split('/')[:-1])+'/'
	...
	while True:
		...
		#store the MODE of the V(D)J genes 
...
def add_to_dict_memory_safe(list_of_files,list_of_filetypes,required_field_names,productivity_function_call,parent_dir,analysis_method):	
	...
	required_field_names = defaultdict(str,get_required_field_names()) #convert this to default dict so that not all fields have to be explicitly defined
	...
	vgene_scores = required_field_names[] 
	jgene_scores = required_field_names[] 
	dgene_scores = required_field_names[] 
	...
	pairing_temp_file = '/'.join(dict_summary.split('/')[:-1])+'/temp_annotation_files.txt'
...
def RunPairing(annotated_file_paths,analysis_method,output_folder_path='',prefix_output_files='', annotated_file_formats=None,field_names=None,cluster_cutoff = [0.96,0.96,0],annotation_cluster_setting=None,use_low_memory = False,files_from_igrep_database=False,productivity_function=None):
	...
	"""						
	if use_low_memory:
		print('The parameter: use_low_memory = True has been deprecated. We now always run the function using the parameter use_low_memory = False. This can be changed in the future if desired')
	...
	if analysis_method=='IMGT':
		if annotated_file_formats == None:			
			...
			imgt_groups = GroupIMGTFiles(annotated_file_paths)		
	...
	if (analysis_method=='IMGT' and annotated_file_formats[0]=='IMGT'):
		...
		for each_group in annotated_file_paths:
			...
			if len(filename)>=3:
				annotated_file_path_summary.append(path+'_'.join(filename[2:]))
			else:
				annotated_file_path_summary.append(path+'_'.join(filename))
	else:
